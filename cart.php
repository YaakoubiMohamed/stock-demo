    ...
    // Check if the $_SESSION['cart'] variable is set
    if (isset($_SESSION['cart'])) {
        // Output the value of $_SESSION['cart'] using var_dump()
    ...
    if (isset($_POST['checkout'])) {
        ...
        $delivery_address = mysqli_real_escape_string(
            $conn,
        ...
        try {
            ...
            $query = "INSERT INTO commande (client_id, num, date, adresse_livraison) VALUES ($user_id, '$order_number', NOW(), '$delivery_address')";
            $conn->query($query);

            // Get the order ID
            $order_id = $conn->insert_id;

            // Insert the items in the cart into the order
            foreach ($_SESSION['cart'] as $article) {
                $article_id = $article['id'];
                $quantity = $article['quantity'];
                $query = "INSERT INTO ligne_commande (commande_id, article_id, quantite) VALUES ($order_id, $article_id, $quantity)";
                $conn->query($query);
            }
    ...
    foreach ($_SESSION['cart'] as $article_id => $quantity) {
        $query = "SELECT designation, prix_unitaire FROM article WHERE id = $article_id";
        $result = $conn->query($query);

        if ($result->num_rows == 1) {
            $row = $result->fetch_assoc();
            $row['quantity'] = $quantity;
            $row['total_price'] = $quantity * $row['prix'];
            $cart_items[] = $row;
            var_dump($cart_items);
        }
    }
                                        ...
                                        <table>
                                            ...
                                            <?php foreach (
                                                ...
                                                <tr>
                                                    ...
                                                    <td data-label="Total" class="ec-cart-pro-subtotal"><?php echo $product[
                                                        'total_price'
                                                    ]; ?></td>
                                ...
                                <div class="ec-cart-summary-bottom">
                                    <div class="ec-cart-summary">
                                        <div class="ec-cart-summary-total">
                                            ...
                                            <span class="text-right"><?php echo $total_price; ?></span>
                                        </div>
                                    </div>
                                    <form action="" method="post">                                                                                
